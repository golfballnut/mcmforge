...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    todo: "bg-stone-800 text-stone-300 border-stone-700",
    in_progress: "bg-orange-900/50 text-orange-300 border-orange-800",
  ...
  const cls = styles[status] ?? "bg-stone-800 text-stone-300 border-stone-700";
  return (
    <span
...
function PriorityBadge({ priority }: { priority: string }) {
  const styles: Record<string, string> = {
    ...
    medium: "bg-orange-900/50 text-orange-300 border-orange-800",
    high: "bg-amber-900/50 text-amber-300 border-amber-800",
    critical: "bg-red-900/50 text-red-300 border-red-800",
  };
  const cls = styles[priority] ?? "bg-stone-800 text-stone-300 border-stone-700";
  return (
    <span
    ...
    <div className="min-h-screen text-stone-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-start justify-between mb-6 md:mb-8">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-400">
              ...
              <span className="text-stone-300 font-medium">{rows.length} total</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Todo", value: counts.todo, color: "text-stone-300" },
            { label: "In Progress", value: counts.in_progress, color: "text-orange-300" },
            { label: "Review", value: counts.review, color: "text-amber-300" },
            { label: "Done", value: counts.done, color: "text-green-300" },
          ].map((stat) => (
            <div
              key={stat.label}
              className="bg-stone-900/50 border border-stone-800 rounded-lg px-5 py-4"
            >
        ...
        {/* Task List */}
        {rows.length === 0 ? (
          <div className="bg-stone-900/50 border border-stone-800 rounded-lg flex flex-col items-center justify-center py-20 text-stone-500">
            <svg
              className="w-10 h-10 mb-3 text-stone-700"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <>
              ...
              {rows.map((task) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <p className="font-medium text-stone-100 text-sm leading-tight">{task.title}</p>
                    <StatusBadge status={task.status} />
                  </div>
                  {task.skill_name && (
                    <p className="text-xs text-stone-500 mb-2">{task.skill_name}</p>
                  )}
            ...
            <div className="hidden md:block bg-stone-900/50 border border-stone-800 rounded-lg overflow-hidden">
                    ...
                    {rows.map((task) => (
                      ...
                      >
                        <td className="px-4 py-3 max-w-xs">
                          <div>
                            <p className="font-medium text-stone-100 truncate">{task.title}</p>
                            {task.skill_name && (
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          {task.assigned_to ? (
                            <span className="text-stone-300">{task.assigned_to}</span>
                          ) : (
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className="text-stone-300">
                            {task.company_registry?.name ?? (
                              <span className="text-stone-600 italic">unknown</span>
                            )}
                          </span>
                        </td>
