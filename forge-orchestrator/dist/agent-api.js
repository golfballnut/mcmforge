...
function matchRoute(url, pattern) {
    ...
    if (urlParts.length !== patParts.length)
        return null;
    const params = {};
...
export function startAgentApi(supabase, port) {
    const server = createServer(async (req, res) => {
        const url = (req.url || '').split('?')[0];
        const method = req.method || 'GET';
        const agentId = req.headers['x-forge-agent-id'];
        ...
        try {
            ...
            if (method === 'POST' && params) {
                ...
                if (!body.body)
                    return json(res, { error: 'Missing body' }, 400);
            ...
            if (method === 'POST' && params) {
                if (!agentId)
                    return json(res, { error: 'Missing x-forge-agent-id' }, 401);
                const body = await parseBody(req);
                const { filename, mimeType, base64, category = 'agent_proof', caption = null, commentId = null } = body;
            ...
            if (method === 'PATCH' && params) {
                ...
                if (body.assignee_agent_id)
                    updates.assignee_agent_id = body.assignee_agent_id;
                const { data, error } = await supabase
                    .from('issues')
                    .update(updates)
                    .eq('id', params.id)
