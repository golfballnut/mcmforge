...
describe('COO router — integration (Supabase mock)', () => {
    ...
    function makeMockSupabase(state) {
        ...
        const build = (table) => ({
            select: (_cols, opts) => {
                const q = { table, op: 'select', count: opts?.count };
                const chain = {
                    eq: (_col, _val) => chain,
                    gte: (_col, _val) => chain,
                    lte: (_col, _val) => chain,
                    ...
                    single: async () => {
                        if (table === 'issues')
                            return { data: state.issue, error: null };
                    ...
                    then: (cb) => {
                        if (table === 'agents')
                            cb({ data: state.agents, error: null });
        ...
        function _val_matcher(_q, _label) { return true; }
        return {
...
//# sourceMappingURL=coo-router.test.js.map