...
export async function startAgentAdvisor(supabase, config) {
    const advise = async () => {
        try {
            ...
            for (const diagnosis of diagnoses) {
                ...
                logger.info({ agent: diagnosis.agentName, issue: diagnosis.issue, severity: diagnosis.severity }, 'Agent Advisor diagnosis');
            }
...
async function analyzeRecentRuns(supabase) {
    ...
    const diagnoses = [];
    const diagnosedAgents = new Set(); // One diagnosis per agent per cycle
    for (const run of runs) {
        const agent = agentMap.get(run.agent_id);
        if (!agent || diagnosedAgents.has(agent.id))
            continue;
        ...
        const allText = [
            ...
            run.stdout_excerpt,
            run.stderr_excerpt,
            run.result_json?.terminal_reason,
            JSON.stringify(run.result_json?.errors),
        ].filter(Boolean).join(' ');
...
async function applyAutoFix(supabase, diagnosis) {
    if (!diagnosis.autoFix)
        return;
    ...
    await supabase
        ...
        .eq('id', diagnosis.agentId);
    logger.info({ agent: diagnosis.agentName, field, oldValue, newValue }, 'Agent Advisor auto-fixed agent config');
}
async function updatePerformanceGrades(supabase) {
...
//# sourceMappingURL=agent-advisor.js.map