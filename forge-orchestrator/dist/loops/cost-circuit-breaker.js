 ...
 * agent.
 */
async function failClosedPauseRecentlyActiveAgents(supabase) {
    const oneHourAgo = new Date(Date.now() - 60 * 60_000).toISOString();
    const { data: recentRuns, error: runsErr } = await supabase
        .from('runs')
        .select('agent_id')
        .gte('finished_at', oneHourAgo);
    if (runsErr || !recentRuns?.length)
        return 0;
    const recentAgentIds = Array.from(new Set(recentRuns.map((r) => r.agent_id)));
    ...
    const { data: paused, error: pauseErr } = await supabase
        ...
        .update({
        status: 'paused',
        pause_reason: 'Circuit breaker fail-closed: cost_events query unreachable. Manual review required before unpause.',
    ...
    if (count > 0) {
        logger.error({ count, names: (paused ?? []).map((p) => p.name) }, 'CIRCUIT BREAKER FAIL-CLOSED — paused recently-active agents due to cost_events unreachable');
    }
    return count;
}
async function computeHourlySpend(supabase) {
    const oneHourAgo = new Date(Date.now() - 60 * 60_000).toISOString();
    ...
    for (const a of agents ?? []) {
        ...
        const cap = a.budget_hourly_cents
...
async function haltAgent(supabase, spend) {
    const spentUsd = (spend.totalCents / 100).toFixed(2);
    const capUsd = (spend.hourlyCapCents / 100).toFixed(2);
    logger.error({ agent: spend.agentName, spentUsd, capUsd }, 'CIRCUIT BREAKER TRIPPED — auto-pausing agent');
    await supabase
        .from('agents')
        .update({
        status: 'paused',
    ...
    const body = `**[COO / CIRCUIT-BREAKER]** Agent **${spend.agentName}** auto-paused — ` +
...
export async function startCostCircuitBreaker(supabase, _config) {
...
//# sourceMappingURL=cost-circuit-breaker.js.map