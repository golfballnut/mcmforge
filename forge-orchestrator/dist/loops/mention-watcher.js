...
export async function startMentionWatcher(supabase, config) {
        ...
        try {
            ...
            for (const comment of comments) {
                ...
                for (const mentionedName of mentions) {
                    const agent = agents.find(a => a.name.toLowerCase() === mentionedName.toLowerCase() &&
                    ...
                    if (agent.status === 'paused' || agent.status === 'terminated')
                        continue;
                    // Don't wake yourself
                    if (agent.id === comment.author_agent_id)
                        continue;
...
//# sourceMappingURL=mention-watcher.js.map