...
export async function startOrphanReaper(supabase, config) {
    const reap = async () => {
        try {
 ...
 */
async function cleanupStaleWakeups(supabase) {
    ...
    if (pausedAgents?.length) {
        const pausedIds = pausedAgents.map((a) => a.id);
    ...
    if (!pending?.length)
        return;
    const issueIds = Array.from(new Set(pending
    ...
    const { data: issues } = await supabase
        ...
        .select('id, status')
        .in('id', issueIds);
    const issuesById = new Map((issues ?? []).map((i) => [i.id, i.status]));
    const wakesToDrop = [];
