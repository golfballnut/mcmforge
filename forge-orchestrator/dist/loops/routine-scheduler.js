...
export async function startRoutineScheduler(supabase, config) {
...
async function processDueRoutines(supabase) {
...
async function fireRoutine(supabase, routine) {
    const agentId = routine.assignee_agent_id;
    if (!agentId) {
    ...
    if (routine.concurrency_policy === 'skip_if_active') {
        ...
        if (agent?.status === 'running') {
            logger.debug({ routineId: routine.id, agentId }, 'Skipping routine — agent is already running (skip_if_active)');
    ...
    logger.info({ routineId: routine.id, routineTitle: routine.title, issueId: issue.id, runId }, 'Routine fired');
    // Advance the schedule
    await advanceNextRunAt(supabase, routine);
}
async function advanceNextRunAt(supabase, routine) {
    const now = new Date().toISOString();
    let nextRun = null;
    if (routine.cron_expression) {
        try {
            ...
            logger.error({ err, routineId: routine.id, cron: routine.cron_expression }, 'Failed to parse cron expression');
        }
    }
    await supabase
        .from('routines')
