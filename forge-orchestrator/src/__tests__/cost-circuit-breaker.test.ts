...
function buildMockClient(opts: {
  costEventsResult: QueryResult<Array<{ agent_id: string; cost_cents: number }>>;
  agentsForSpend?: Array<{ id: string; name: string; company_id: string; budget_hourly_cents?: number | null; status?: string }>;
  recentRuns?: Array<{ agent_id: string }>;
  pausedAgents?: Array<{ id: string; name: string }>;
}) {
...
describe('cost-circuit-breaker tick', () => {
  ...
  it('trips an agent when hourly spend exceeds cap (normal path)', async () => {
    ...
    const result = await tick(client as never);
  ...
  it('on SECOND consecutive query error: fail-closed pauses recently-active agents', async () => {
    ...
    const result2 = await tick(client as never);
    ...
    const pauseCall = updateCalls.find(
      (c) => c.table === 'agents' && (c.payload as { status?: string }).status === 'paused',
    );
