...
type SupabaseMock = ReturnType<typeof buildMockClient>['client'];

function buildMockClient(opts: {
  existingComments?: Array<{ id: string }>;
  agentUpdateError?: boolean;
}) {
        ...
        return {
          select: () => ({
            eq: () => ({
              eq: () => ({
                limit: () =>
                  Promise.resolve({ data: opts.existingComments ?? [], error: null }),
              }),
            }),
          }),
        };
...
describe('checkAndPauseOnQuotaCap', () => {
  ...
  it('returns false and does NOT pause when run duration >= 10s', async () => {
    ...
    const result = await checkAndPauseOnQuotaCap(
      client as never,
      'agent-1',
      'run-1',
      'issue-1',
  ...
  it('auto-pauses with correct duration in the reason', async () => {
    const { client } = buildMockClient({ existingComments: [] });
    await checkAndPauseOnQuotaCap(client as never, 'agent-1', 'run-1', 'issue-1', 4_321);
