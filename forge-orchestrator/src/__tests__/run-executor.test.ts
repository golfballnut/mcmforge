...
function buildSupabase(recentContinueCount: number) {
...
async function executeWithRecentContinueCount(recentContinueCount: number) {
  ...
  await executeRun(
    supabase as never,
    { dryRun: false, agentHomeDir: '/tmp/forge-orchestrator-run-executor-test' } as never,
    makeRun(),
    makeAgent(),
  );
