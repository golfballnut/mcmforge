...
describe('judgeImages() — file validation', () => {
  it('throws if mockup file does not exist', async () => {
    await expect(
      judgeImages('/nonexistent/mockup.png', afterPassPath, tmpDir),
    ).rejects.toThrow(/Visual Judge: mockup not found/);
  });

  it('throws if after-image does not exist', async () => {
    await expect(
      judgeImages(mockupPath, '/nonexistent/after.png', tmpDir),
    ).rejects.toThrow(/Visual Judge: after-image not found/);
  });
});
