...
// Only these agent ids (or human steve_user_id) may emit gate/dispatch/approval tags.
// Seeded at startup from env or config; in the future: a role table.
export interface COOConfig {
  ceoAgentIds: Set<string>; // agent UUIDs allowed to post [GATE-*], [DISPATCH-OK]
  stevenUserIdPrefix: string; // e.g. 'steve' or a supabase auth uid
}

export type ParsedTag =
  | { type: 'gate_passed'; gate: number }
  | { type: 'gate_failed'; gate: number }
  | { type: 'dispatch_ok' }
  | { type: 'approved' }
  | { type: 'proof' }
  | { type: 'blocked_mention'; agentName: string };

/**
 * Pure function: given a comment body, return the meta-tags detected.
 * Exported for unit testing.
 */
export function parseMetaTags(body: string): ParsedTag[] {
  const tags: ParsedTag[] = [];

  const gp = body.match(TAG_PATTERNS.gatePassed);
  if (gp) tags.push({ type: 'gate_passed', gate: parseInt(gp[1], 10) });

  const gf = body.match(TAG_PATTERNS.gateFailed);
  if (gf) tags.push({ type: 'gate_failed', gate: parseInt(gf[1], 10) });

  if (TAG_PATTERNS.dispatchOk.test(body)) tags.push({ type: 'dispatch_ok' });
  if (TAG_PATTERNS.approved.test(body)) tags.push({ type: 'approved' });
  if (TAG_PATTERNS.proof.test(body)) tags.push({ type: 'proof' });

  const bl = body.match(TAG_PATTERNS.blockedAt);
  if (bl) tags.push({ type: 'blocked_mention', agentName: bl[1].trim() });
 ...
 */
export async function validateProofHasAttachment(
  supabase: SupabaseClient,
  comment: CommentRow,
): Promise<{ valid: boolean; attachmentCount: number }> {
 ...
 */
export async function getAgentGate(supabase: SupabaseClient, agentId: string): Promise<number> {
...
async function postCOOComment(
  ...
  if (error) logger.warn({ err: error, body: body.slice(0, 80) }, 'COO: failed to post routing comment');
}
 ...
 */
export async function executeGatePassed(
  supabase: SupabaseClient,
  comment: CommentRow,
  newGate: number,
  cooConfig: COOConfig,
  ...
  const authorIsAuthorized =
    (comment.author_agent_id && cooConfig.ceoAgentIds.has(comment.author_agent_id)) ||
  ...
  if (!agent) return `gate-passed tag ignored — assignee agent not found`;
...
export async function executeGateFailed(
  supabase: SupabaseClient,
  comment: CommentRow,
  failedGate: number,
  cooConfig: COOConfig,
  ...
  if (!authorIsAuthorized) return `gate-failed tag ignored — author not authorized`;
  ...
  if (!issue?.assignee_agent_id) return `gate-failed tag ignored — issue has no assignee`;
  ...
  if (!agent) return `gate-failed tag ignored — assignee agent not found`;
...
export async function executeBlockedMention(
  supabase: SupabaseClient,
  comment: CommentRow,
  ...
  if (!target) return `blocked-mention ignored — no agent named "${mentionedAgentName}" in this company`;
...
export async function executeProofValidation(
  supabase: SupabaseClient,
  comment: CommentRow,
): Promise<string | null> {
...
export async function executeDispatchOk(
  supabase: SupabaseClient,
  comment: CommentRow,
  cooConfig: COOConfig,
): Promise<string | null> {
  const authorIsAuthorized =
    (comment.author_agent_id && cooConfig.ceoAgentIds.has(comment.author_agent_id)) ||
    (comment.author_user_id?.includes(cooConfig.stevenUserIdPrefix));
  if (!authorIsAuthorized) return `dispatch-ok tag ignored — author not authorized`;
...
export async function executeApproved(
  ...
  const authorIsAuthorized =
    (comment.author_agent_id && cooConfig.ceoAgentIds.has(comment.author_agent_id)) ||
    (comment.author_user_id?.includes(cooConfig.stevenUserIdPrefix));
  if (!authorIsAuthorized) return `approved tag ignored — author not authorized`;
...
export async function evaluateComment(
  ...
  for (const tag of tags) {
    try {
      ...
      if (tag.type === 'gate_passed') summary = await executeGatePassed(supabase, comment, tag.gate, cooConfig);
      else if (tag.type === 'gate_failed') summary = await executeGateFailed(supabase, comment, tag.gate, cooConfig);
      else if (tag.type === 'dispatch_ok') summary = await executeDispatchOk(supabase, comment, cooConfig);
      else if (tag.type === 'approved') summary = await executeApproved(supabase, comment, cooConfig);
      else if (tag.type === 'proof') summary = await executeProofValidation(supabase, comment);
      else if (tag.type === 'blocked_mention') summary = await executeBlockedMention(supabase, comment, tag.agentName);

      if (summary) actions.push(summary);
    } catch (err) {
      logger.error({ err, tag, commentId: comment.id }, 'COO: action execution failed');
      actions.push(`action failed for tag ${tag.type}: ${(err as Error).message}`);
    }
  }
...
export async function tick(
  supabase: SupabaseClient,
  cooConfig: COOConfig,
): Promise<{ commentsProcessed: number; actionsTaken: number }> {
  ...
  for (const comment of newComments) {
    ...
    if (comment.author_user_id === 'coo-router') continue;

    const actions = await evaluateComment(supabase, comment as CommentRow, cooConfig);
    if (actions.length > 0) {
...
export async function startCOORouter(supabase: SupabaseClient, config: ForgeConfig): Promise<void> {
  ...
  const cooConfig: COOConfig = {
    ceoAgentIds: new Set((ceoAgents ?? []).map(a => a.id)),
    stevenUserIdPrefix: 'steve',
  };

  logger.info(
    { ceoAgentCount: cooConfig.ceoAgentIds.size, interval: 30_000 },
    'COO router started',
  );
