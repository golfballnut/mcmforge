...
import { tick, parseMetaTags, type COOConfig } from './coo-router.js';

type Check = { name: string; pass: boolean; note?: string };
const checks: Check[] = [];
const add = (name: string, pass: boolean, note?: string) => {
...
async function main() {
  ...
  add('parse: [GATE-PASSED 2]',
    parseMetaTags('[GATE-PASSED 2] dry-run clean').some(t => t.type === 'gate_passed' && t.gate === 2));
  add('parse: [DISPATCH-OK]',
    parseMetaTags('[DISPATCH-OK] go').some(t => t.type === 'dispatch_ok'));
  add('parse: [PROOF] with attachments heading',
    parseMetaTags('[PROOF] done\n## Artifacts\n- url').some(t => t.type === 'proof'));
  add('parse: [BLOCKED] @Agent Name stops at first lowercase',
    parseMetaTags('[BLOCKED] cannot build. @Map Rendering Expert please fix tomorrow')
  ...
  const perCommentResults: Array<{ body: string; actions: string[] }> = [];
  for (const cid of commentIds) {
    const { data: cmRow } = await supabase.from('issue_comments').select('*').eq('id', cid).single();
    if (!cmRow) continue;
    const actions = await evaluateComment(supabase as any, cmRow as any, cooConfig);
  ...
  add('S6 dispatch-ok recorded as issue_event',
    /dispatch-ok recorded/.test(s6Actions));
