...
const GATE_BLOCKERS: Record<number, string[]> = {
  0: [
...
interface RunStats {
  ...
  lastFailedRun: { id: string; summary: string | null; created_at: string } | null;
}

async function fetchAgents(supabase: SupabaseClient): Promise<AgentRow[]> {
...
async function fetchRunStats(supabase: SupabaseClient, agentId: string, sinceIso: string): Promise<RunStats> {
  const { data } = await supabase
    .from('runs')
...
async function ensureCertificationIssue(supabase: SupabaseClient, agent: AgentRow): Promise<{ id: string; identifier: string; created: boolean }> {
  // Look for existing
  ...
  if (!newIssue) throw new Error(`failed to create Certification issue for ${agent.name}`);
  return { id: newIssue.id, identifier: newIssue.identifier, created: true };
}

function lessonsCountFor(agent: AgentRow, repoRoot: string): { count: number; file: string | null } {
  if (!agent.instructions_file) return { count: 0, file: null };
  ...
  if (!existsSync(lessonsFile)) return { count: 0, file: lessonsFile };
  try {
...
function formatDigest(agent: AgentRow, stats: RunStats, lessons: { count: number }): string {
...
async function main() {
  ...
  if (nameFilter) agents = agents.filter(a => a.name.toLowerCase().includes(nameFilter));
