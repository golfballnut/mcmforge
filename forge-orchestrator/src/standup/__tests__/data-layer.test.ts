...
vi.mock('node:child_process', async () => {
  ...
  return {
    ...
    execSync: vi.fn().mockReturnValue(
      Buffer.from(
        '2026-05-05T10:00:00Z|feat: ship FORGE-360 standup card|Steve McMillian\n' +
...
function makeSupabase(opts: {
  ...
  runs?: Array<{ id: string; status: string; cost_usd: string }>;
  issuesError?: string;
  runsError?: string;
}) {
  ...
  const from = vi.fn().mockImplementation((table: string) => {
    if (table === 'issues') return issueChain;
    if (table === 'runs') return runChain;
...
describe('fetchStandupData', () => {
  ...
  it('returns issue count and open issues for last 24h', async () => {
    ...
    const result = await fetchStandupData(sb as never, COMPANY_ID);
